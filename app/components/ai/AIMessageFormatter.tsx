...
export const AIMessageFormatter = ({ content, isUser = false }: AIMessageFormatterProps) => {
  ...
  // 用户消息：右对齐，纯文本显示，浅色背景 + 蓝色边框
  if (isUser) {
    return (
      <div className="mb-4 flex justify-end items-start gap-2">
        <div className="flex flex-col items-end max-w-[85%]">
          <div className="flex items-center gap-1.5 mb-1">
            <User size={14} className="text-[var(--text-quaternary)]" />
            <span className="text-xs text-[var(--text-quaternary)] font-medium">你</span>
          </div>
          <div className="bg-white rounded-lg p-3 text-sm shadow-sm">
            <p className="whitespace-pre-wrap break-words leading-relaxed text-[var(--text-primary)]">{content}</p>
          </div>
        </div>
      </div>
    );
  ...
  // AI 消息：左对齐，Markdown 渲染，浅色背景
  return (
    <div className="mb-4 flex justify-start items-start gap-2">
      <div className="flex flex-col items-start max-w-[85%]">
        <div className="flex items-center gap-1.5 mb-1">
          <Bot size={14} className="text-[var(--text-quaternary)]" />
          <span className="text-xs text-[var(--text-quaternary)] font-medium">AI 助手</span>
        </div>
        <div className="bg-[var(--layer-bg-gray)] rounded-lg p-3 text-sm border border-[var(--border-quaternary)] shadow-sm">
          <ReactMarkdown
