...
export const Nav = ({
    ...
    return (
        <TooltipProvider>
            <header className="shrink-0 px-6 flex items-center justify-between bg-transparent h-12" style={{ height: '48px' }}>
                {/* Left - Logo */}
                ...
                <div className="flex items-center">
                    <button
                        ...
                        className="relative flex items-center gap-2 px-3 py-2 rounded-lg transition-opacity hover:opacity-90 group"
                        title={showAIPanel ? 'Hide AI Panel' : 'Show AI Panel'}
                    >
                        {/* Claude Image with Color Overlay */}
                        <div className="relative w-6 h-6">
                            ...
                            <div
                                className={`absolute inset-0 rounded transition-colors ${showAIPanel
                                    ? 'bg-[var(--light-blue-60)]/20'
                                    : 'bg-gray-400/20'
                                    }`}
                            />
                        </div>
                        {/* Tooltip-like text */}
                        <span className="text-xs font-semibold text-[var(--text-secondary)] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
